...
class FEMDiscreteShellBase(SimulationBase):
    ...
    def add_particle_3D(self, boxLen, num, translate, rotCenter, rotAxis, rotDeg, scale):
        meshCounter = FEM.DiscreteShell.Add_Discrete_Particles(boxLen, num, self.X, self.discrete_particle)
        MeshIO.Transform_Points(translate, rotCenter, rotAxis, rotDeg, scale, meshCounter, self.X)
        return meshCounter
    ...
    def advance_one_time_step(self, dt):
        ...
        if self.lv_fn >= 0:
            ...
            FEM.DiscreteShell.Initialize_Shell_Hinge_EIPC(2000, 1, 0.4, 0.01, self.dt, self.dHat2, newX, self.Elem, self.segs, \
                self.edge2tri, self.edgeStencil, self.edgeInfo, self.nodeAttr, self.massMatrix, self.gravity, self.bodyForce, \
                self.elemAttr, self.elasticity, self.kappa)
            self.initialize_OIPC(1e-3, 0)
