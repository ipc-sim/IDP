...
public:
    ...
    double& value
        ...
        else if (PARAMETER::Get("Elasticity_model", std::string("")).substr(0, 2) == std::string("SD")) {
            T p = stod(PARAMETER::Get("Elasticity_model", std::string("")).substr(2));
            SYMMETRIC_DIRICHLET_FUNCTOR<T, dim>::Compute_Psi(elasticityAttr, h * h, elemAttr, value, p);
        }
    ...
    T dHat2, T kappa[]
        ...
        if (PARAMETER::Get("Elasticity_model", std::string("")) == std::string("FCR"))
            ...
            T p = stod(PARAMETER::Get("Elasticity_model", std::string("")).substr(2));
            SYMMETRIC_DIRICHLET_FUNCTOR<T, dim>::Compute_First_PiolaKirchoff_Stress(elasticityAttr, h * h, elemAttr, p);
        }
        else if (PARAMETER::Get("Elasticity_model", std::string("")) == std::string("SH"))
    ...
    std::vector<Eigen::Triplet<T>>& triplets
        ...
        if (PARAMETER::Get("Elasticity_model", std::string("")) == std::string("FCR"))
            ...
            T p = stod(PARAMETER::Get("Elasticity_model", std::string("")).substr(2));
            SYMMETRIC_DIRICHLET_FUNCTOR<T, dim>::Compute_First_PiolaKirchoff_Stress_Derivative(elasticityAttr, h * h, true, dP_div_dF, p);
        }
        else if (PARAMETER::Get("Elasticity_model", std::string("")) == std::string("SH"))
