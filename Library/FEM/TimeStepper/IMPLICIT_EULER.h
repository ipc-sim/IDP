    ...
    MESH_ELEM<2>& Extra_Mesh)
{
    ...
    if (current_frame == 1 &&
        PARAMETER::Get("Output_Psi", std::string("")) == std::string("YES") &&
        PARAMETER::Get("Elasticity_model", std::string("")).substr(0, 2) == std::string("SD"))
    {
        std::string filePath = output_folder + "/distortion0.obj";
    ...
    do {
        ...
        if (infNorm / h < NewtonTol) {
            if ((PNIter == 1) && PARAMETER::Get("Exit_When_Done", true)) {
                PNIter = 0;
            }
    ...
    if (PARAMETER::Get("Output_Psi", std::string("")) == std::string("YES") &&
        PARAMETER::Get("Elasticity_model", std::string("")).substr(0, 2) == std::string("SD"))
    {
        int atLeast = std::stoi(PARAMETER::Get("Output_Psi_AtLeast", std::string("")));
        if (PNIter >= atLeast) {
