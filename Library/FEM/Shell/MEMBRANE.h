    ...
    if constexpr (dim == 2) {
        ...
        Elem.Join(elasticityAttr).Each([&](int id, auto data) {
            ...
            if constexpr (useNH) {
                ...
                E += h * h * vol * (mu / 2 * ((IB * A).trace() - dim - 2 * lnJ) + lambda / 2 * lnJ * lnJ);
            }
            else {
    ...
    std::vector<Eigen::Triplet<T>>& triplets)
{
    ...
    if constexpr (dim == 2) {
        ...
        int newTripletStart = triplets.size();
        triplets.resize(newTripletStart + Elem.size * 81);
        Elem.Join(elasticityAttr).Par_Each([&](int id, auto data) {
            ...
            for (int i = 0; i < 9; ++i) {
                int tripletIStart = newTripletStart + id * 81 + i * 9;
                for (int j = 0; j < 9; ++j) {
    ...
    FIXED_COROTATED<T, dim - 1>& elasticityAttr)
{
    ...
    T E0 = 0;
    Compute_Membrane_Energy(Elem, 1.0, X, nodeAttr, elemAttr, elasticityAttr, E0); 
    nodeAttr.template Fill<FIELDS<MESH_NODE_ATTR<T, dim>>::g>(VECTOR<T, dim>(0));
    Compute_Membrane_Gradient(Elem, 1.0, X, nodeAttr, elemAttr, elasticityAttr); 
    ...
    FIXED_COROTATED<T, dim - 1>& elasticityAttr)
{
    ...
    Compute_Membrane_Gradient(Elem, 1.0, X, nodeAttr0, elemAttr, elasticityAttr);
    std::vector<Eigen::Triplet<T>> HStriplets;
    Compute_Membrane_Hessian(Elem, 1.0, false, X, nodeAttr, elemAttr, elasticityAttr, HStriplets); 
    CSR_MATRIX<T> HS;
    ...
    for (int i = 0; i < X.size * dim; ++i) {
        ...
        nodeAttr.template Fill<FIELDS<MESH_NODE_ATTR<T, dim>>::g>(VECTOR<T, dim>(0));
        Compute_Membrane_Gradient(Elem, 1.0, Xperturb, nodeAttr, elemAttr, elasticityAttr);
