    ...
    if constexpr (dim == 2) {
        ...
        if constexpr (KL) {
            Elem.Join(elasticityAttr).Each([&](int id, auto data) {
                ...
                const MATRIX<T, dim - 1>& IB = std::get<FIELDS<MESH_ELEM_ATTR<T, dim>>::IB>(elemAttr.Get_Unchecked(id));
    ...
    if constexpr (dim == 2) {
        ...
        if constexpr (KL) {
            ...
            if (elemAttr.size) {
                ...
                const T k = bendingStiffMult * std::get<FIELDS<MESH_ELEM_ATTR<T, dim>>::P>(elemAttr.Get_Unchecked(0))(0, 0);
                int newTripletStartI = triplets.size();
                triplets.resize(newTripletStartI + edgeStencil.size() * 12 * 12);
                threads.Par_Each([&](int eI, auto data) {
                    ...
                    for (int i = 0; i < 12; ++i) {
                        int startI = newTripletStartI + eI * 12 * 12 + i * 12;
                        for (int j = 0; j < 12; ++j) {
    ...
    FIXED_COROTATED<T, dim - 1>& elasticityAttr)
{
    ...
    T E0 = 0;
    Compute_Bending_Energy<T, dim, KL>(Elem, 1.0, edge2tri, edgeStencil, edgeInfo, thickness, T(1), X, nodeAttr, elemAttr, elasticityAttr, E0); 
    nodeAttr.template Fill<FIELDS<MESH_NODE_ATTR<T, dim>>::g>(VECTOR<T, dim>(0));
    Compute_Bending_Gradient<T, dim, KL>(Elem, 1.0, edge2tri, edgeStencil, edgeInfo, thickness, T(1), X, nodeAttr, elemAttr, elasticityAttr); 
    ...
    for (int i = 0; i < X.size * dim; ++i) {
        ...
        T E = 0;
        Compute_Bending_Energy<T, dim, KL>(Elem, 1.0, edge2tri, edgeStencil, edgeInfo, thickness, T(1), Xperturb, nodeAttr, elemAttr, elasticityAttr, E);
        grad_FD[i] = (E - E0) / eps;
    }
    ...
    FIXED_COROTATED<T, dim - 1>& elasticityAttr)
{
    ...
    Compute_Bending_Gradient<T, dim, KL>(Elem, 1.0, edge2tri, edgeStencil, edgeInfo, thickness, T(1), X, nodeAttr0, elemAttr, elasticityAttr);
    std::vector<Eigen::Triplet<T>> HStriplets;
    Compute_Bending_Hessian<T, dim, KL>(Elem, 1.0, false, edge2tri, edgeStencil, edgeInfo, thickness, T(1), X, nodeAttr, elemAttr, elasticityAttr, HStriplets); 
    CSR_MATRIX<T> HS;
    ...
    for (int i = 0; i < X.size * dim; ++i) {
        ...
        nodeAttr.template Fill<FIELDS<MESH_NODE_ATTR<T, dim>>::g>(VECTOR<T, dim>(0));
        Compute_Bending_Gradient<T, dim, KL>(Elem, 1.0, edge2tri, edgeStencil, edgeInfo, thickness, T(1), Xperturb, nodeAttr, elemAttr, elasticityAttr);
