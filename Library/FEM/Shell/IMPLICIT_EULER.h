    ...
    T& DBCStiff, T& Eprev, T& alpha, T& feasibleAlpha)
{
    ...
    if (kappa_s[0] > 0 || fiberStiffMult[0] > 0) {
        do {
            ...
            valid = Compute_IncPotential<T, dim, KL, elasticIPC>(Elem, h, edge2tri, edgeStencil, edgeInfo, thickness, bendingStiffMult, fiberStiffMult, 
    ...
    if (withCollision) {
        Compute_Intersection_Free_StepSize<T, dim, false, elasticIPC>(X, boundaryNode, boundaryEdge, boundaryTri, 
            particle, rod, codimBNStartInd, DBCb, sol, thickness, alpha); // CCD
    ...
    do {
        ...
        valid = Compute_IncPotential<T, dim, KL, elasticIPC>(Elem, h, edge2tri, edgeStencil, edgeInfo, thickness, bendingStiffMult, fiberStiffMult, fiberLimit,
        ...
        if (valid) {
            if (withCollision) {
                Compute_Constraint_Set<T, dim, false, elasticIPC>(X, nodeAttr, boundaryNode, boundaryEdge, boundaryTri, 
                    particle, rod, BNArea, BEArea, BTArea, codimBNStartInd, DBCb, dHat2, thickness, false, constraintSet, constraintSetPTEE, stencilInfo);
                T minDist2;
                std::vector<T> dist2;
    ...
    const std::string& outputFolder)
{
    ...
    if (withCollision) {
        Compute_Intersection_Free_StepSize<T, dim, false, elasticIPC>(X, boundaryNode, boundaryEdge, boundaryTri, 
            particle, rod, codimBNStartInd, DBCb, DBCDisp, thickness, DBCAlpha);
    ...
    T Eprev;
    Compute_IncPotential<T, dim, KL, elasticIPC>(Elem, h, edge2tri, edgeStencil, edgeInfo, thickness, bendingStiffMult, fiberStiffMult, fiberLimit,
        s, sHat, kappa_s, DBCb, X, Xtilde, nodeAttr, M, elemAttr, elasticityAttr, 
        withCollision, constraintSet, stencilInfo, dHat2, kappa, staticSolve, b, 
        tet, tetAttr, tetElasticityAttr, rod, rodInfo, rodHinge, rodHingeInfo, 
    ...
    do {
        ...
        // compute gradient
        Compute_IncPotential_Gradient<T, dim, KL, elasticIPC>(Elem, h, edge2tri, edgeStencil, edgeInfo, thickness, bendingStiffMult, fiberStiffMult, fiberLimit,
        ...
        // compute Hessian
        if (!useGD) {
            Compute_IncPotential_Hessian<T, dim, KL, elasticIPC>(Elem, h, edge2tri, edgeStencil, edgeInfo, thickness, bendingStiffMult, fiberStiffMult, fiberLimit,
        ...
        Line_Search<T, dim, KL, elasticIPC>(Elem, seg, DBC, edge2tri, edgeStencil, edgeInfo, 
            ...
            boundaryNode, boundaryEdge, boundaryTri, BNArea, BEArea, BTArea, codimBNStartInd, 
        ...
        if (DBCStiff) {
            ...
            if(progress < 0.99) {
                ...
                Compute_IncPotential<T, dim, KL, elasticIPC>(Elem, h, edge2tri, edgeStencil, edgeInfo, thickness, bendingStiffMult, fiberStiffMult, 
                    fiberLimit, s, sHat, kappa_s, DBCb, X, Xtilde, nodeAttr, M, elemAttr, elasticityAttr, 
                    withCollision, constraintSet, stencilInfo, dHat2, kappa, staticSolve, b, 
                    tet, tetAttr, tetElasticityAttr, rod, rodInfo, rodHinge, rodHingeInfo, 
                    stitchInfo, stitchRatio, k_stitch, Eprev);
    ...
    if (Elem.size) {
        Compute_Max_And_Avg_Stretch(Elem, staticSolve ? 1.0 : h, fiberStiffMult, X, nodeAttr, elemAttr, elasticityAttr, maxs, avgs, minc, avgc);
