    ...
    std::vector<int>& compNodeRange)
{
    ...
    MESH_ELEM<dim - 1> newElem;
    Read_TriMesh_Tex_Obj(filePath, newX, newXTex, newElem, stitchNodes, stitchRatio);
    ...
    if constexpr (dim == 2) {
        ...
        Elem.Each([&](int id, auto data) {
            ...
            for (int endI = 0; endI < dim; ++endI) {
                ...
                b[elemVInd[endI] * dim + 1] += massPortion * gravity[1];
            }
        });
        if (Elem.size) {
...
void Export_Discrete_Shell(py::module& m) {
    ...
    shell_m.def("Initialize_OIPC_VM", &Initialize_OIPC_VecM<double, 2>);
    shell_m.def("Initialize_OIPC_VM", &Initialize_OIPC_VecM<double, 3>);
