    ...
    T& value)
{
    ...
    if (fiberStiffMult[0] > 0 || fiberStiffMult[1] > 0) {
        ...
        Compute_Membrane_Energy(Elem, staticSolve ? 1.0 : h, X, nodeAttr, elemAttr, elasticityAttr, value);
    }
    Compute_Bending_Energy<T, dim, KL>(Elem, staticSolve ? 1.0 : h, edge2tri, edgeStencil, edgeInfo, thickness, bendingStiffMult, X, nodeAttr, elemAttr, elasticityAttr, value);
    
    // volumetric elasticity
    Compute_Deformation_Gradient(X, tet, tetAttr, tetElasticityAttr);
    FIXED_COROTATED_FUNCTOR<T, dim>::Compute_Psi(tetElasticityAttr, h * h, tetAttr, value);
    ...
    // garment
    Compute_Stitch_Energy(X, stitchInfo, stitchRatio, k_stitch, h, value);
    ...
    T k_stitch)
{
    ...
    if (fiberStiffMult[0] > 0 || fiberStiffMult[1] > 0) {
        ...
        Compute_Membrane_Gradient(Elem, staticSolve ? 1.0 : h, X, nodeAttr, elemAttr, elasticityAttr);
    }
    if (kappa_s[0] > 0) {
        Compute_Inextensibility_Gradient(Elem, staticSolve ? 1.0 : h, s, sHat, kappa_s, DBCb, X, nodeAttr, elemAttr, elasticityAttr);
    }
    Compute_Bending_Gradient<T, dim, KL>(Elem, staticSolve ? 1.0 : h, edge2tri, edgeStencil, edgeInfo, thickness, bendingStiffMult, X, nodeAttr, elemAttr, elasticityAttr);
    
    // volumetric elasticity
    FIXED_COROTATED_FUNCTOR<T, dim>::Compute_First_PiolaKirchoff_Stress(tetElasticityAttr, h * h, tetAttr);
    ...
    // garment
    Compute_Stitch_Gradient(X, stitchInfo, stitchRatio, k_stitch, h, nodeAttr);
    ...
    CSR_MATRIX<T>& sysMtr)
{
    ...
    if (fiberStiffMult[0] > 0 || fiberStiffMult[1] > 0) {
        ...
        Compute_Membrane_Hessian(Elem, staticSolve ? 1.0 : h, projectSPD, X, nodeAttr, elemAttr, elasticityAttr, triplets);
    }
    if (kappa_s[0] > 0) {
    ...
    Compute_Bending_Hessian<T, dim, KL>(Elem, staticSolve ? 1.0 : h, projectSPD, edge2tri, edgeStencil, edgeInfo, thickness, bendingStiffMult, X, nodeAttr, elemAttr, elasticityAttr, triplets);
    ...
    // volumetric elasticity:
    {
        ...
        FIXED_COROTATED_FUNCTOR<T, dim>::Compute_First_PiolaKirchoff_Stress_Derivative(tetElasticityAttr, h * h, projectSPD, dP_div_dF);
    ...
    // garment
    Compute_Stitch_Hessian(X, stitchInfo, stitchRatio, k_stitch, h, projectSPD, triplets);
