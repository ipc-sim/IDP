    ...
    MESH_NODE_ATTR<T, dim>& nodeAttr)
{
    if constexpr (allocNA) {
        ...
        for (int nodeI = 0; nodeI < X.size; ++nodeI) {
            VECTOR<T, dim> supp;
            supp(0) += PARAMETER::Get("Initial_Collision_Velocity", (T)0.) * (nodeI < X.size / 2 ? 1. : -1.);
            nodeAttr.Append(std::get<0>(X.Get_Unchecked(nodeI)), velocity + supp, VECTOR<T, dim>(), 0.0);
        }
    }
    ...
    Elem.Each([&](int id, auto data) {
        ...
        if (PARAMETER::Get("final_flower_fruit_id", 0)) {
            int id = PARAMETER::Get("final_flower_fruit_id", 0);
            T rho = PARAMETER::Get("final_flower_fruit_rho", (T)0);
            if (elemVInd(0) >= id) {
                massEach = volI * rho / (dim + 1);
            }
        }
