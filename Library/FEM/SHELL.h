    ...
    DBC_MOTION<T, dim>& DBCMotion)
{
    ...
    Init_Dirichlet(X_mesh, relBoxMin, relBoxMax, v, rotCenter, rotAxis, angVelDeg, (T)0., DBC_mesh, DBCMotion_mesh);
    ...
    if constexpr (dim == 2) {
        ...
        DBCMotion.Append(VECTOR<int, 2>(DBCSize0, DBC.size), v, rotCenter, rotAxis, angVelDeg, 0);
        ...
        DBCMotion.Append(VECTOR<int, 2>(DBCSize0, DBC.size), VECTOR<T, dim>(0), 
            VECTOR<T, dim>(0), rotAxis, angVelDeg, 0);
    }
    else {
        int DBCSize0 = DBC.size;
        DBC_mesh.Each([&](int id, auto data) {
    ...
    MESH_NODE_ATTR<T, dim>& nodeAttr_mesh)
{
    ...
    if (withCollision) {
        Compute_Constraint_Set<T, dim, true>(X_mesh, nodeAttr_mesh, boundaryNode, boundaryEdge, boundaryTri, 
            std::vector<int>(), std::vector<VECTOR<int, 2>>(), BNArea, BEArea, BTArea,
    ...
    do {
        ...
        if (withCollision) {
            ...
            Compute_Intersection_Free_StepSize<T, dim, true>(X_mesh, boundaryNode, boundaryEdge, boundaryTri, 
                std::vector<int>(), std::vector<VECTOR<int, 2>>(), VECTOR<int, 2>(boundaryNode.size(), boundaryNode.size()), 
