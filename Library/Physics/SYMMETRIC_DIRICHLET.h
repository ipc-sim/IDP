...
public:
    ...
    static void Output_Psi(const std::string& filePath, 
        MESH_NODE<T, dim>& X, MESH_ELEM<dim>& Elem, STORAGE& sd) 
    {
        TIMER_FLAG("OutputPsi");
        std::vector<T> psi(sd.size);
        ...
        if (out) {
            ...
            for (const auto& psiI : psi) {
                fprintf(out, "vt %le 0\n", psiI);
                if constexpr (dim == 3) {
                    fprintf(out, "vt %le 0\n", psiI);
                    fprintf(out, "vt %le 0\n", psiI);
                    fprintf(out, "vt %le 0\n", psiI);
                    // one tet has 4 triangles
                }
            }
            Elem.Each([&](const int i, auto data) {
                auto& [elemVInd] = data;
                if constexpr (dim == 2) {
                    fprintf(out, "f %d/%d %d/%d %d/%d\n", 
                        elemVInd[0] + 1, i + 1, elemVInd[1] + 1, i + 1, elemVInd[2] + 1, i + 1);
                }
                else {
                    fprintf(out, "f %d/%d %d/%d %d/%d\n", 
                        elemVInd[0] + 1, i * 4 + 1, elemVInd[2] + 1, i * 4 + 1, elemVInd[1] + 1, i * 4 + 1);
                    fprintf(out, "f %d/%d %d/%d %d/%d\n", 
                        elemVInd[0] + 1, i * 4 + 2, elemVInd[3] + 1, i * 4 + 2, elemVInd[2] + 1, i * 4 + 2);
                    fprintf(out, "f %d/%d %d/%d %d/%d\n", 
                        elemVInd[0] + 1, i * 4 + 3, elemVInd[1] + 1, i * 4 + 3, elemVInd[3] + 1, i * 4 + 3);
                    fprintf(out, "f %d/%d %d/%d %d/%d\n", 
                        elemVInd[1] + 1, i * 4 + 4, elemVInd[2] + 1, i * 4 + 4, elemVInd[3] + 1, i * 4 + 4);
                }
            });
            fclose(out);
        }
    ...
    static void Compute_Psi(STORAGE& sd, T w, MESH_ELEM_ATTR<T, dim>& elemAttr, T& Psi, T p = 1) {
        ...
        sd.Par_Each([&](const int i, auto data) {
          ...
          if (p != 1) {
            psi[i] = std::pow((2 * mu + lambda) / 8 * dim * 2, 1 - p) / p * std::pow(psi[i], p);
          }
    ...
    static void Compute_First_PiolaKirchoff_Stress(STORAGE& sd, T w, MESH_ELEM_ATTR<T, dim>& elemAttr, T p = 1) {
        ...
        sd.Par_Each([&](const int i, auto data) {
          ...
          if (p != 1) {
            T psi;
            Compute_E(sigma, u, lambda, psi);
            dE_div_dsigma *= std::pow((2 * u + lambda) / 8 * dim * 2, 1 - p) * std::pow(psi, p - 1);
          }
    ...
    static void Compute_First_PiolaKirchoff_Stress_Derivative(STORAGE& sd, T w, bool projectSPD, DIFFERENTIAL& dP_div_dF, T p = 1) {
        ...
        sd.Join(dP_div_dF).Par_Each([&](const int i, auto data) {
          ...
          Compute_DP_Div_DF(U, sigma, V, mu, lambda, w * vol, projectSPD && (p == 1), dP_div_dFI);

          if (p != 1) {
            T psi;
            Compute_E(sigma, mu, lambda, psi);

            VECTOR<T, dim> dE_div_dsigma;
            Compute_DE_Div_DSigma(sigma, mu, lambda, dE_div_dsigma);
            MATRIX<T, dim> dpsi_div_dF = U * MATRIX<T, dim>(dE_div_dsigma) * V.transpose();
            Eigen::Matrix<T, dim * dim, 1> dpsi_div_dF_vec;
            for (int i = 0; i < dim; ++i) {
                for (int j = 0; j < dim; ++j) {
                    dpsi_div_dF_vec[i + j * dim] = dpsi_div_dF(i, j);
                }
            }

            Eigen::Matrix<T, dim * dim, dim * dim> hess = std::pow((2 * mu + lambda) / 8 * dim * 2, 1 - p) * std::pow(psi, p - 2) * 
                (w * vol * (p - 1) * dpsi_div_dF_vec * dpsi_div_dF_vec.transpose() + psi * dP_div_dFI);
            if (projectSPD) {
                makePD(hess);
            }
            dP_div_dFI = hess;
