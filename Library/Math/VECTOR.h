...
public:
    ...
    JGSL_FORCE_INLINE T min() const {
        if constexpr (dim == 1) return data[0];
        if constexpr (dim == 2) return std::min(data[0], data[1]);
        if constexpr (dim == 3) return std::min(std::min(data[0], data[1]), data[2]);
        if constexpr (dim == 4) return std::min(std::min(data[0], data[1]), std::min(data[2], data[3]));
    }

    JGSL_FORCE_INLINE VECTOR<T, dim> exp() const {
        VECTOR<T, dim> v;
        for (int d = 0; d < dim; ++d) v(d) = std::exp(data[d]);
        return v;
    }
...
template <class T> JGSL_FORCE_INLINE T cross(const VECTOR<T, 2>& a, const VECTOR<T, 2>& b) { return a[0] * b[1] - a[1] * b[0]; }
